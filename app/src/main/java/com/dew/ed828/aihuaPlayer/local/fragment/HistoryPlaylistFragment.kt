...
import com.dew.ed828.aihuaPlayer.fragments.BaseLocalListFragment
...
class HistoryPlaylistFragment : BaseLocalListFragment<List<StreamStatisticsEntry>, Void>() {
    ...
    private val historyObserver: Subscriber<List<StreamStatisticsEntry>>
        get() = object : Subscriber<List<StreamStatisticsEntry>> {
            ...
            override fun onError(exception: Throwable) {
                this@HistoryPlaylistFragment.onError(exception)
            }
    ...
    protected fun processResult(results: List<StreamStatisticsEntry>): List<StreamStatisticsEntry>? {
        return when (sortMode) {
            HistoryPlaylistFragment.StatisticSortMode.LAST_PLAYED -> {
                Collections.sort(results) { left, right -> right.latestAccessDate.compareTo(left.latestAccessDate) }
                results
            }
            HistoryPlaylistFragment.StatisticSortMode.MOST_PLAYED -> {
