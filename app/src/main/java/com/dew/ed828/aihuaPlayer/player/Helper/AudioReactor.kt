...
class AudioReactor(private val context: Context,
...
) : AudioManager.OnAudioFocusChangeListener, AudioRendererEventListener {
    ...
    init {
        player.addAudioDebugListener(this) //todo: @deprecated Use {@link #addAnalyticsListener(AnalyticsListener)} to get more detailed debug information.
    ...
    fun dispose() {
        abandonAudioFocus()
        player.removeAudioDebugListener(this)  // todo:  @deprecated Use {@link #removeAnalyticsListener(AnalyticsListener)} to get more detailed debug information.
    }
    ...
    fun requestAudioFocus() {
        if (SHOULD_BUILD_FOCUS_REQUEST) {
            ...
            audioManager.requestAudioFocus(this, STREAM_TYPE, FOCUS_GAIN_TYPE) //todo: @deprecated use {@link #requestAudioFocus(AudioFocusRequest)}
        }
    }
    ...
    fun abandonAudioFocus() {
        if (SHOULD_BUILD_FOCUS_REQUEST) {
            ...
            audioManager.abandonAudioFocus(this) // todo: @deprecated use {@link #abandonAudioFocusRequest(AudioFocusRequest)}
        }
    }
    ...
    private fun animateAudio(from: Float, to: Float) {
        ...
        valueAnimator.duration = AudioReactor.DUCK_DURATION
