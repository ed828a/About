...
import com.dew.ed828.aihuaPlayer.util.SliderStrategy
import kotlinx.android.synthetic.main.dialog_playback_parameter.view.*
...
class PlaybackParameterDialog : DialogFragment() {
    ...
    private val currentTempo: Double
        get() = if (tempoSlider == null)
            ...
            strategy.valueOf(tempoSlider!!.progress)

    private val currentPitch: Double
        get() = if (pitchSlider == null)
            pitch
        else
            strategy.valueOf(pitchSlider!!.progress)
    ...
    private fun setupTempoControl(rootView: View) {
        tempoSlider = rootView.playbackParameterTempoSeekbar
        val tempoMinimumText = rootView.playbackParameterTempoMinimumText
        val tempoMaximumText = rootView.playbackParameterTempoMaximumText
        tempoCurrentText = rootView.playbackParameterTempoCurrentText
        tempoStepUpText = rootView.playbackParameterTempoStepUp
        tempoStepDownText = rootView.playbackParameterTempoStepDown
    ...
    private fun setupPitchControl(rootView: View) {
        pitchSlider = rootView.playbackParameterPitchSeekbar
        val pitchMinimumText = rootView.playbackParameterPitchMinimumText
        val pitchMaximumText = rootView.playbackParameterPitchMaximumText
        pitchCurrentText = rootView.playbackParameterPitchCurrentText
        pitchStepDownText = rootView.playbackParameterPitchStepDown
        pitchStepUpText = rootView.playbackParameterPitchStepUp
    ...
    private fun setupHookingControl(rootView: View) {
        unhookingCheckbox = rootView.playbackParameterUnhookCheckbox
    ...
    private fun setupSkipSilenceControl(rootView: View) {
        skipSilenceCheckbox = rootView.playbackParameterSkipSilenceCheckbox
    ...
    private fun setupStepSizeSelector(rootView: View) {
        val stepSizeOnePercentText = rootView.playbackParameterStepSizeOnePercent
        val stepSizeFivePercentText = rootView.playbackParameterStepSizeFivePercent
        val stepSizeTenPercentText = rootView.playbackParameterStepSizeTenPercent
        val stepSizeTwentyFivePercentText = rootView.playbackParameterStepSizeTwentyFivePercent
        val stepSizeOneHundredPercentText = rootView.playbackParameterStepSizeOneHundredPercent

        if (stepSizeOnePercentText != null) {
            stepSizeOnePercentText.text = getPercentString(STEP_ONE_PERCENT_VALUE)
            stepSizeOnePercentText.setOnClickListener { view -> changeStepSize(STEP_ONE_PERCENT_VALUE) }
        }

        if (stepSizeFivePercentText != null) {
            stepSizeFivePercentText.text = getPercentString(STEP_FIVE_PERCENT_VALUE)
            stepSizeFivePercentText.setOnClickListener { view -> changeStepSize(STEP_FIVE_PERCENT_VALUE) }
        }

        if (stepSizeTenPercentText != null) {
            stepSizeTenPercentText.text = getPercentString(STEP_TEN_PERCENT_VALUE)
            stepSizeTenPercentText.setOnClickListener { view -> changeStepSize(STEP_TEN_PERCENT_VALUE) }
        }

        if (stepSizeTwentyFivePercentText != null) {
            stepSizeTwentyFivePercentText.text = getPercentString(STEP_TWENTY_FIVE_PERCENT_VALUE)
            stepSizeTwentyFivePercentText.setOnClickListener { view -> changeStepSize(STEP_TWENTY_FIVE_PERCENT_VALUE) }
        }

        if (stepSizeOneHundredPercentText != null) {
            stepSizeOneHundredPercentText.text = getPercentString(STEP_ONE_HUNDRED_PERCENT_VALUE)
            stepSizeOneHundredPercentText.setOnClickListener { view -> changeStepSize(STEP_ONE_HUNDRED_PERCENT_VALUE) }
        }
    }
