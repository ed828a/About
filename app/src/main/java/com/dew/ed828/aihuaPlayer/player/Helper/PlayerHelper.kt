...
import com.dew.ed828.aihuaPlayer.player.Helper.PlayerHelper.MinimizeMode.Companion.MINIMIZE_ON_EXIT_MODE_BACKGROUND
import com.dew.ed828.aihuaPlayer.player.Helper.PlayerHelper.MinimizeMode.Companion.MINIMIZE_ON_EXIT_MODE_NONE
import com.dew.ed828.aihuaPlayer.player.Helper.PlayerHelper.MinimizeMode.Companion.MINIMIZE_ON_EXIT_MODE_POPUP
...
object PlayerHelper {
    ...
    fun getTimeString(milliSeconds: Int): String {
        val seconds = milliSeconds % 60000L / 1000L
        val minutes = milliSeconds % 3600000L / 60000L
        val hours = milliSeconds % 86400000L / 3600000L
        val days = milliSeconds % (86400000L * 7L) / 86400000L
    ...
    fun mimeTypesOf(format: SubtitlesFormat): String =
        when (format) {
            SubtitlesFormat.VTT -> MimeTypes.TEXT_VTT
            SubtitlesFormat.TTML -> MimeTypes.APPLICATION_TTML
            else -> throw IllegalArgumentException("Unrecognized mime type: " + format.name)

        }

    fun captionLanguageOf(context: Context, subtitles: Subtitles): String {
        val displayName = subtitles.locale.getDisplayName(subtitles.locale)
        val caption = if (subtitles.isAutoGenerated) " (${context.getString(R.string.caption_auto_generated)})"
                             else ""
        return displayName + caption
    }

    @SuppressLint("SwitchIntDef")
    fun resizeTypeOf(context: Context, @AspectRatioFrameLayout.ResizeMode resizeMode: Int): String =
        when (resizeMode) {
            RESIZE_MODE_FIT -> context.resources.getString(R.string.resize_fit)
            RESIZE_MODE_FILL -> context.resources.getString(R.string.resize_fill)
            RESIZE_MODE_ZOOM -> context.resources.getString(R.string.resize_zoom)
    ...
    fun cacheKeyOf(info: StreamInfo, video: VideoStream): String =
        info.url + video.getResolution() + video.getFormat().getName()


    fun cacheKeyOf(info: StreamInfo, audio: AudioStream): String =
        info.url + audio.averageBitrate + audio.getFormat().getName()
     ...
     * checked. If it is non-null and is not part of the existing items, then
    ...
    fun autoQueueOf(info: StreamInfo, existingItems: List<PlayQueueItem>): PlayQueue? {
        ...
        val relatedItems = info.relatedStreams
    ...
    fun isResumeAfterAudioFocusGain(context: Context): Boolean = isResumeAfterAudioFocusGain(context, false)


    fun isVolumeGestureEnabled(context: Context): Boolean = isVolumeGestureEnabled(context, true)


    fun isBrightnessGestureEnabled(context: Context): Boolean = isBrightnessGestureEnabled(context, true)


    fun isUsingOldPlayer(context: Context): Boolean = isUsingOldPlayer(context, false)


    fun isRememberingPopupDimensions(context: Context): Boolean = isRememberingPopupDimensions(context, true)


    fun isAutoQueueEnabled(context: Context): Boolean = isAutoQueueEnabled(context, false)
    ...
    fun getPreferredCacheSize(context: Context): Long = 64 * 1024 * 1024L


    fun getPreferredFileSize(context: Context): Long = 512 * 1024L


    /**
     * Returns the number of milliseconds the player buffers for before starting playback.
     */
    fun getPlaybackStartBufferMs(context: Context): Int = 500
     ...
     * playback.
     */
    fun getPlaybackMinimumBufferMs(context: Context): Int = 25000
     ...
     */
    fun getPlaybackOptimalBufferMs(context: Context): Int = 60000


    fun getQualitySelector(context: Context, meter: BandwidthMeter): TrackSelection.Factory =
    ...
    fun isUsingDSP(context: Context): Boolean = true


    fun getTossFlingVelocity(context: Context): Int = 2500
    ...
    private fun isResumeAfterAudioFocusGain(context: Context, boolean: Boolean): Boolean {
        return getPreferences(context).getBoolean(context.getString(R.string.resume_on_audio_focus_gain_key), boolean)
    }
    ...
    private fun setScreenBrightness(context: Context, screenBrightness: Float, timestamp: Long) {
        ...
        with(editor){
            ...
            putLong(context.getString(R.string.screen_brightness_timestamp_key), timestamp)
            apply()
        }
    }
    ...
    private fun getMinimizeOnExitAction(context: Context, key: String): String? {
