...
import com.dew.ed828.aihuaPlayer.player.Helper.PlayerHelper.isUsingOldPlayer
...
class PopupVideoPlayer : Service() {
        ...
        public override fun onFullScreenButtonClicked() {
            ...
            if (!isUsingOldPlayer(applicationContext)) {
                intent = NavigationHelper.getPlayerIntent(
                    context,
                    MainVideoPlayerActivity::class.java,
                    this.playQueue!!,
                    this.repeatMode,
                    this.playbackSpeed,
                    this.playbackPitch,
    ...
    private inner class PopupWindowGestureListener : GestureDetector.SimpleOnGestureListener(), View.OnTouchListener {
        ...
        override fun onScroll(initialEvent: MotionEvent, movingEvent: MotionEvent, distanceX: Float, distanceY: Float): Boolean {
            ...
            Log.d(TAG, "PopupVideoPlayer.onScroll = , e1.getRaw = [${initialEvent.rawX}, " +
                    "${initialEvent.rawY}], e1.getX,Y = [${initialEvent.x}, ${initialEvent.y}], " +
                    "e2.getRaw = [${movingEvent.rawX}, ${movingEvent.rawY}], e2.getX,Y = [${movingEvent.x}, ${movingEvent.y}], " +
                    "distanceX,Y = [$distanceX, $distanceY], posX,Y = [$posX, $posY], popupW,H = [$popupWidth x $popupHeight]"
            )
        ...
        override fun onTouch(v: View, event: MotionEvent): Boolean {
            ...
            if (event.action == MotionEvent.ACTION_MOVE && !isMoving && isResizing) {
                Log.d(TAG, "onTouch() ACTION_MOVE > v = [$v],  e1.getRaw = [${event.rawX}, ${event.rawY}]")
    ...
    companion object {
        ...
        const val ACTION_CLOSE = "com.dew.ed828.aihuaPlayer.player.PopupVideoPlayer.CLOSE"
        const val ACTION_PLAY_PAUSE = "com.dew.ed828.aihuaPlayer.player.PopupVideoPlayer.PLAY_PAUSE"
        const val ACTION_REPEAT = "com.dew.ed828.aihuaPlayer.player.PopupVideoPlayer.REPEAT"
