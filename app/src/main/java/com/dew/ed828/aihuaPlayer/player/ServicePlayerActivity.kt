...
import com.dew.ed828.aihuaPlayer.player.Helper.PlayerHelper.formatSpeed
import com.dew.ed828.aihuaPlayer.player.playqueue.PlayQueueItem
import com.dew.ed828.aihuaPlayer.player.playqueue.PlayQueueItemBuilder
import com.dew.ed828.aihuaPlayer.player.playqueue.PlayQueueItemHolder
import com.dew.ed828.aihuaPlayer.player.playqueue.PlayQueueItemTouchCallback
...
import kotlinx.android.synthetic.main.activity_player_queue_control.view.*
...
abstract class ServicePlayerActivity : AppCompatActivity(), PlayerEventListener, SeekBar.OnSeekBarChangeListener, View.OnClickListener, PlaybackParameterDialog.Callback {
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.action_switch_main -> {
                ...
                applicationContext.startActivity(getSwitchIntent(MainVideoPlayerActivity::class.java))
                return true
            }
        }
    ...
    private fun buildQueue() { // filling up RecyclerView List
    ...
    private fun buildMetadata() {
        ...
        metadataTitle = rootView!!.findViewById(R.id.songName)
        metadataArtist = rootView!!.findViewById(R.id.artistName)
    ...
    private fun buildSeekBar() {
        progressCurrentTime = rootView!!.currentTime
        progressSeekBar = rootView!!.findViewById(R.id.seekBar)
        progressEndTime = rootView!!.findViewById(R.id.endTime)
        progressLiveSync = rootView!!.findViewById(R.id.liveSync)
        seekDisplay = rootView!!.findViewById(R.id.seekDisplay)
    ...
    private fun scrollToSelected() {
        ...
        val currentVisibleIndex: Int = if (itemsList!!.layoutManager is LinearLayoutManager) {
    ...
    override fun onClick(view: View) {
        ...
        when {
            view.id == repeatButton!!.id -> player!!.onRepeatClicked()
            view.id == backwardButton!!.id -> player!!.onPlayPrevious()
            view.id == playPauseButton!!.id -> player!!.onPlayPause()
            view.id == forwardButton!!.id -> player!!.onPlayNext()
            view.id == shuffleButton!!.id -> player!!.onShuffleClicked()
            view.id == playbackSpeedButton!!.id -> openPlaybackParameterDialog()
            view.id == playbackPitchButton!!.id -> openPlaybackParameterDialog()
            view.id == metadata!!.id -> scrollToSelected()
            view.id == progressLiveSync!!.id -> player!!.seekToDefault()
        }
    }
    ...
    override fun onProgressUpdate(currentProgress: Int, duration: Int, bufferPercent: Int) {
        ...
        // Set current time if not isSeeking
        if (!isSeeking) {
    ...
    private fun onStateChanged(state: Int) {
        when (state) {
            ...
            BasePlayer.STATE_COMPLETED -> playPauseButton!!.setImageResource(R.drawable.ic_replay_white)
                ...
                Log.d(getTag(), "onStateChanged() received a wrong state: $state")
            }
        }
