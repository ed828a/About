...
abstract class PlayQueue internal constructor(index: Int, startWith: List<PlayQueueItem>) : Serializable {
    ...
    var streams: ArrayList<PlayQueueItem> = ArrayList()
    ...
    /**
     * broadcastReceiver: Returns the play queue's update broadcast.
    ...
    /**
     * isComplete: Checks if the queue is complete.
     *
    ...
    /**
     * set(index): Changes the current playing index to a new index.
    ...
    private val selfReporter: Subscriber<PlayQueueEvent>
        get() = object : Subscriber<PlayQueueEvent> {
            ...
            override fun onError(throwable: Throwable) {
                Log.e(TAG, "Received broadcast error", throwable)
            }
    ...
    fun getItem(index: Int): PlayQueueItem? =
        if ( index < 0 || index >= streams.size || streams[index] == null)
            null
        else
            streams[index]
    ...
    private fun removeInternal(removeIndex: Int) {
        ...
        backup?.let {
            val backupIndex = it.indexOf(getItem(removeIndex))
            it.removeAt(backupIndex)
        }
