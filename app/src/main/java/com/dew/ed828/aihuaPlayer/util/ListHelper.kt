...
object ListHelper {
     ...
     * @see .getDefaultResolutionIndex
     *
     * This function get the default Resolution Index of a certain videoStreams
     */
     ...
     * @see .getDefaultResolutionIndex
     *
     * in fact, this function is exactly doing the same operation as getResolutionIndex(...), so it's videoStreams to decide the Resolution
     */
    ...
    private fun computeDefaultResolution(context: Context, key: Int, value: Int): String {
        ...
        if (maxResolution != null && resolution != null && compareVideoStreamResolution(maxResolution, resolution) < 1) {
     ...
     * @return the sorted list
     */
    internal fun getSortedStreamVideosList(defaultFormat: MediaFormat?,
                                           showHigherResolutions: Boolean,
                                           videoStreams: List<VideoStream>?, videoOnlyStreams: List<VideoStream>?, ascendingOrder: Boolean): List<VideoStream> {
    ...
    private fun compareAudioStreamBitrate(streamA: AudioStream?, streamB: AudioStream?,
        ...
        return if (streamA.averageBitrate > streamB.averageBitrate) {
            ...
            formatRanking.indexOf(streamA.getFormat()) - formatRanking.indexOf(streamB.getFormat())
        }
    }
    ...
    private fun compareVideoStreamResolution(streamA: VideoStream?, streamB: VideoStream?): Int {
        ...
        return if (resComp != 0) {
            ...
            // Same bitrate and format
            ListHelper.VIDEO_FORMAT_QUALITY_RANKING.indexOf(streamA.getFormat()) - ListHelper.VIDEO_FORMAT_QUALITY_RANKING.indexOf(streamB.getFormat())
        }
    }
