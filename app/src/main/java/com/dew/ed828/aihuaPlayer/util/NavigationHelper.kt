...
/**
 *
 * Created by Edward on 12/8/2018.
 *
 */
...
//    @JvmOverloads
    ...
    fun getPlayerEnqueueIntent(
        context: Context?,
        targetClazz: Class<*>,
        playQueue: PlayQueue,
        selectOnAppend: Boolean
    ): Intent {
    ...
    fun playOnMainPlayer(context: Context?, queue: PlayQueue) {
        val playerIntent = getPlayerIntent(context, MainVideoPlayerActivity::class.java, queue)
    ...
    fun playOnOldVideoPlayer(context: Context, info: StreamInfo) {
        ...
        if (index == -1) {
            Toast.makeText(context, R.string.video_streams_empty, Toast.LENGTH_LONG).show()
            return
        }

        val videoStream = videoStreamsList[index]
        val intent = Intent(context, PlayVideoActivity::class.java)   // old Video Player Activity
            ...
            if (context is Activity) {
                AlertDialog.Builder(context)
                    ...
                    .setPositiveButton(R.string.install) { dialog, which ->
                        ...
                        intent.data = Uri.parse(context.getString(R.string.fdroid_vlc_url))
                        context.startActivity(intent)
                    }
    ...
    fun openChannel(context: Context, serviceId: Int, url: String, name: String? = null) {
        ...
        if (name != null && !name.isEmpty()) openIntent.putExtra(KEY_TITLE, name)
    ...
    fun openVideoDetail(context: Context, serviceId: Int, url: String, title: String? = null) {
        ...
        if (title != null && !title.isEmpty()) openIntent.putExtra(KEY_TITLE, title)
        context.startActivity(openIntent)
    }
    ...
    private fun getServicePlayerActivityIntent(
        context: Context,
        activityClass: Class<*>
    ): Intent {
    ...
    fun getIntentByLink(context: Context?, service: StreamingService, url: String?): Intent {
        ...
        when (linkType) {
            StreamingService.LinkType.STREAM -> rIntent.putExtra(
                VideoDetailFragment.AUTO_PLAY,
